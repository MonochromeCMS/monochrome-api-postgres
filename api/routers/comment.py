...
async def _get_comment(comment_id: UUID, db_session: AsyncSession = Depends(get_db)):
...
async def create_comment(
    ...
    if payload.reply_to:
        reply_comment = await Comment.find(db_session, payload.reply_to, None)
...
async def delete_comment(
    comment: Comment = Permission("edit", _get_comment), db_session: AsyncSession = Depends(get_db)
):
