...
tag ?= ghcr.io/monochromecms/monochrome-api-postgres
buildkit ?= 1
test_exit ?= 0
user ?= `id -u`
dir ?= `pwd`
...
up-db: ## Start a db container
	@docker run --rm --name monochrome-db -e POSTGRES_PASSWORD=postgres -p 5432:5432 -d postgres:13-alpine
	@ip=`docker inspect --format='{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' monochrome-db`
	@echo DB_URL=postgresql+asyncpg://postgres:postgres@$$ip:5432/postgres
